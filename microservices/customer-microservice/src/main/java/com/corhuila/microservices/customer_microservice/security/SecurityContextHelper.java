...
public class SecurityContextHelper {
    ...
    public CurrentUserContext requireAuthenticated() {
        ...
        if (isBlank(context.userId()) || isBlank(context.username())) {
            throw new CustomerSecurityException(HttpStatus.UNAUTHORIZED, "Authentication is required");
        }
        return context;
    }
    ...
    public CurrentUserContext requireTenant() {
        ...
        if (isBlank(context.tenantId())) {
            throw new CustomerSecurityException(HttpStatus.FORBIDDEN, "Tenant context is required");
        }
        return context;
    }
    ...
    public CurrentUserContext requirePermission(String permission) {
        ...
        if (!context.permissions().contains(permission)) {
            throw new CustomerSecurityException(HttpStatus.FORBIDDEN, "Required permission is missing");
        }
        return context;
    }
