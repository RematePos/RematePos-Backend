...
## Versionado MongoDB (customer/cart)
Este repositorio versiona PostgreSQL con Liquibase y MongoDB con scripts idempotentes.
- Changelogs Mongo: `mongo/changelog/*.js`
- Runner Mongo: `mongo/run-migrations.sh`
- Historial aplicado: DB `db_migrations`, collection `applied_migrations`
Al levantar `docker-compose/docker-compose.yml`, se ejecutan:
1) `liquibase` para PostgreSQL
2) `mongo-migrate` para MongoDB
Validacion rapida de Mongo:
```powershell
docker exec -it rematepos-mongodb mongosh -u admin -p admin123 --authenticationDatabase admin --eval "db.getSiblingDB('customer_db').customers.getIndexes()"
docker exec -it rematepos-mongodb mongosh -u admin -p admin123 --authenticationDatabase admin --eval "db.getSiblingDB('db_migrations').applied_migrations.find().toArray()"
```
