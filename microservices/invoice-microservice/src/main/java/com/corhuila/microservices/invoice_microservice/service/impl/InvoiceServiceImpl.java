...
public class InvoiceServiceImpl implements InvoiceService {
    ...
    public InvoiceGenerateResponse generate(InvoiceGenerateRequest request) {
        return repository.findByPurchaseId(request.purchaseId())
    ...
    public InvoiceResponse getByInvoiceNumber(String invoiceNumber) {
        return repository.findByInvoiceNumber(normalizeInvoiceNumber(invoiceNumber))
                .map(this::toResponse)
                .orElseThrow(() -> new NoSuchElementException("Invoice not found"));
    }
    ...
    public List<InvoiceResponse> getByCustomerId(String customerId) {
        return repository.findByCustomerIdOrderByIssuedAtDesc(customerId)
                .stream()
                .map(this::toResponse)
                .toList();
    }
    ...
    public List<InvoiceResponse> getByDocument(String documentType, String documentNumber) {
        return repository.findByCustomerDocumentTypeAndCustomerDocumentNumberOrderByIssuedAtDesc(documentType, documentNumber)
                .stream()
                .map(this::toResponse)
                .toList();
    }
    ...
    public List<InvoiceResponse> getRecent(int limit) {
        ...
        return repository.findAllByOrderByIssuedAtDesc(PageRequest.of(0, safeLimit))
                .stream()
                .map(this::toResponse)
                .toList();
    }
